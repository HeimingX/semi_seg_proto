...
def get_loader(cfg, seed=0, distributed_flag=True):
    cfg_dataset = cfg["dataset"]

    if cfg_dataset["type"] == "cityscapes_semi":
        train_loader_sup, train_loader_unsup = build_city_semi_loader(
            "train", cfg, seed=seed, distributed_flag=distributed_flag
        )
        val_loader = build_cityloader("val", cfg, distributed_flag=distributed_flag)
        ...
        val_loader = build_vocloader("val", cfg, distributed_flag=distributed_flag)
        logger.info("Get loader Done...")
        return train_loader_sup, train_loader_unsup, val_loader

    elif cfg_dataset["type"] == "pascal":
        train_loader_sup = build_vocloader("train", cfg, seed=seed, distributed_flag=distributed_flag)
        val_loader = build_vocloader("val", cfg, distributed_flag=distributed_flag)
        logger.info("Get loader Done...")
        return train_loader_sup, val_loader
