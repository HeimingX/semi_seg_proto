...
class ToTensor(object):
    ...
    def __call__(self, image, label):
        ...
        image = Tensor(image.transpose((2, 0, 1))[np.newaxis])
        if not isinstance(image, mindspore.float32):
            image = image.float()
        label = Tensor(label[np.newaxis, np.newaxis])
        if not isinstance(label, mindspore.float32):
            label = label.float()
        return image, label
...
class Normalize(object):
    ...
    def __init__(self, mean, std=None):
        if std is None:
            ...
            self.std = Tensor(np.float32(std)[:, np.newaxis, np.newaxis])
        self.mean = Tensor(np.float32(mean)[:, np.newaxis, np.newaxis])
...
class Resize(object):
    ...
    def __call__(self, image, label):
        image = P.ResizeBilinear(image, size=self.size, align_corners=False)
        # OR:
        # image = ops.interpolate(image, roi=None, scales=None, sizes=self.size, coordinate_transformation_mode="align_corners", mode="bilinear")
        label = P.ResizeBilinear(label, size=self.size, align_corners=False)  # current mindspore does not have nearest mode, but this have problems!!!!
...
class ResizeLongSize(object):
    ...
    def __call__(self, image, label):
        ...
        if h > w:
            ...
            image = P.ResizeBilinear(image, size=(self.size, w_r), align_corners=False)
            label = P.ResizeBilinear(label, size=(self.size, w_r), align_corners=False)
            # label = F.interpolate(label, size=(self.size, w_r), mode="nearest")
        else:
            h_r = int(2048 * h / w)
            image = P.ResizeBilinear(image, size=(h_r, self.size), align_corners=False)
            label = P.ResizeBilinear(label, size=(h_r, self.size), align_corners=False)
...
class RandResize(object):
    ...
    def __call__(self, image, label):
        ...
        image = P.ResizeBilinear(image, size=(new_h, new_w), align_corners=False)
        label = P.ResizeBilinear(label, size=(new_h, new_w), align_corners=False)
...
class Crop(object):
    ...
    def __call__(self, image, label):
        ...
        if pad_h > 0 or pad_w > 0:
            ...
            label = ops.pad(label, border, mode="constant", value=self.ignore_label)
        h, w = image.size()[-2:]
...
class RandRotate(object):
    ...
    def __call__(self, image, label):
        ...
        # TODO: mindspore transfer to be done
        pass
...
class RandomHorizontalFlip(object):
    def __call__(self, image, label):
        if random.random() < 0.5:
            # https://mindspore.cn/docs/api/en/r1.3/api_python/ops/mindspore.ops.ReverseV2.html#mindspore.ops.ReverseV2
            op_flip = ops.ReverseV2(axis=[3])
            image = op_flip(image)
            label = op_flip(label)
        return image, label
...
class GaussianBlur(nn.Module):
    def __init__(self, radius):
        ...
        self.kernel = nn.Conv2d(
            3, 3, self.kernel_size, stride=1, padding=self.radius, has_bias=False, groups=3
        )
        self.weight_init()
    ...
    def weight_init(self):
        ...
        for param in self.kernel.parameters():
            param.data.copy_(Tensor(weight))
